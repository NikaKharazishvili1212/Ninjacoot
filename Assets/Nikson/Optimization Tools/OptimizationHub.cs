...
namespace Nikson
{
    ...
    public partial class OptimizationHub : EditorWindow
    {
        ...
        void OnEnable()
        {
            ...
            textureTools = CreateInstance<TextureTools>();
            analyzer = CreateInstance<Analyzer>();
        }
        ...
        void OnGUI()
        {
            ...
            selectedTab = GUILayout.SelectionGrid(selectedTab, TAB_LABELS, 4, GUILayout.Height(40));
            // Clear things on new tab open
            if (selectedTab != lastTab)
            {
                lastTab = selectedTab;
                GUI.FocusControl(null);
                textureTools.ClearTextures();
            ...
            switch (selectedTab)
            {
                case 0: meshCombiner.OnGUI(); break;
                case 1: uvAtlasMapper.OnGUI(); break;
                case 2: textureTools.OnGUI(); break;
                case 3: analyzer.OnGUI(); break;
            }
        ...
        // Draws the shared info icon above the description label — called at the top of each tool's OnGUI()
        public static void DrawIcon() => EditorGUILayout.LabelField("ℹ", GetStyle());
        ...
        // Shared by MeshUvAtlasMapper and TextureTools — stores original texture import settings so they can be restored after processing
