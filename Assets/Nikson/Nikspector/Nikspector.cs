...
namespace Nikson
{
    /// <summary>Draws a button in the Inspector labeled by the method name.</summary>
    [AttributeUsage(AttributeTargets.Method)]
    public class ButtonAttribute : Attribute { }

    /// <summary>Groups fields and buttons into named tabs at the top of the Inspector.</summary>
    ...
    /// <summary>Groups fields under a collapsible foldout section. Use with [FoldoutEnd] to close it.</summary>
    ...
    /// <summary>Closes the most recent [Foldout] section.</summary>
    [AttributeUsage(AttributeTargets.Field)]
    public class FoldoutEndAttribute : Attribute { }

    /// <summary>Shows static, const, or readonly fields and properties in the Inspector. Plain fields show no prefix; properties show (property); arrays show (array); lists show (list); const/readonly/static fields keep their qualifier.</summary>
    [AttributeUsage(AttributeTargets.Field | AttributeTargets.Property)]
    public class BetterSerializerAttribute : Attribute { }

    /// <summary>Draws a bold centered white header above the next field, shown exactly once regardless of multi-field declarations or placement.</summary>
    [AttributeUsage(AttributeTargets.All, AllowMultiple = true)]
    public class BetterHeaderAttribute : Attribute
    {
        public string Text;
        public BetterHeaderAttribute(string text) => Text = text;
    }
    ...
    public class NikspectorData : ScriptableObject
    {
        ...
        public class FieldDisplayInfo
        {
            ...
            public string BetterHeader; // null if no header precedes this field
        }
    ...
    public class NikspectorMonoBehaviourEditor : Editor
    {
        ...
        static GUIStyle _betterHeaderStyle;

        static GUIStyle BetterHeaderStyle
        {
            get
            {
                if (_betterHeaderStyle == null)
                {
                    _betterHeaderStyle = new GUIStyle(EditorStyles.boldLabel)
                    {
        ...
        void SetupFields()
        {
            ...
            string currentTab = "";
            var assignedHeaders = new HashSet<string>();
            ...
            foreach (var member in allMembers)
            {
                ...
                var betterHeaderAttr = member.GetCustomAttribute<BetterHeaderAttribute>();
                string pendingHeader = null;
                if (betterHeaderAttr != null && assignedHeaders.Add(betterHeaderAttr.Text))
                    pendingHeader = betterHeaderAttr.Text;
                ...
                if (member is FieldInfo field)
                {
                    ...
                    {
                        bool isList = field.FieldType.IsGenericType && field.FieldType.GetGenericTypeDefinition() == typeof(List<>);
                        bool isArray = field.FieldType.IsArray;
                        if (isList || isArray)
                        {
                            Debug.LogWarning($"[BetterSerializer] is not supported on arrays or lists (field: {field.Name}). Use [SerializeField] instead.");
                            continue;
                        }
        ...
        void DrawFieldList(List<NikspectorData.FieldDisplayInfo> fields)
        {
            ...
            foreach (var field in fields)
            {
                ...
                // Draw BetterHeader BEFORE entering the foldout, at the current (outer) indent level
                if (field.BetterHeader != null && _data.RootFoldout.IsSubfoldoutContentVisible(drawingFoldoutPath))
                {
                    UpdateIndentLevel(drawingFoldoutPath);
                    GUILayout.Space(6);
                    EditorGUILayout.LabelField(field.BetterHeader, BetterHeaderStyle);
                    var rect = GUILayoutUtility.GetLastRect();
                    EditorGUI.DrawRect(new Rect(rect.x, rect.yMax + 1, rect.width, 1f), new Color(1f, 1f, 1f, 0.25f));
                    GUILayout.Space(4);
                }
        ...
        void DrawBetterSerializerMember(NikspectorData.FieldDisplayInfo fieldInfo)
        {
            ...
            if (fieldInfo.Property != null)
            {
                bool isStatic = fieldInfo.Property.GetGetMethod(true)?.IsStatic == true;
                label = isStatic
                    ? $"(static property) {fieldInfo.Name}"
                    : $"(property) {fieldInfo.Name}";
            }
            else
            {
                var f = fieldInfo.Field;
                bool isList = memberType.IsGenericType && memberType.GetGenericTypeDefinition() == typeof(List<>);
                bool isArray = memberType.IsArray;

                if (f.IsLiteral) label = $"(const) {fieldInfo.Name}";
                else if (f.IsInitOnly) label = $"(readonly) {fieldInfo.Name}";
                else if (f.IsStatic) label = $"(static) {fieldInfo.Name}";
                else if (isArray) label = $"(array) {fieldInfo.Name}";
                else if (isList) label = $"(list) {fieldInfo.Name}";
                else label = fieldInfo.Name;
            }
            ...
            // Arrays and Lists: draw as normal serialized field but with custom label
            if (fieldInfo.Field != null && fieldInfo.SerializedProp != null)
            {
                EditorGUILayout.PropertyField(fieldInfo.SerializedProp, new GUIContent(label), true);
                return;
            }
